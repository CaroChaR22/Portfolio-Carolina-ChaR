export const colors = {
    TEXT_GRADIENT_PURPLE: "linear-gradient(135deg,#cb5eee 0%,#4be1ec 100%)",
    TEXT_GRADIENT_RED: "linear-gradient(135deg, #D92546 0%, #e675dc 50%, #fd5ddb 90%)",
    // TEXT_GRADIENT_RED: "linear-gradient(135deg, #55585D 0%, #C9C9C9 50%, #E3E3E3 90%)",
    TEXT_GRADIENT_GREEN: "linear-gradient(135deg,#d6ff7f 0%,#00b3cc 100%)",
    TEXT_GREY_NEUTRAL: "#6b6f76",
    TEXT_BLACK: "#0D0D0D",
    TEXT_WHITE: "#fff",
  
    BACKGROUND_GRADIENT_PINK:
      "linear-gradient( 135deg, rgb(102,153,255) 0%, rgb(255,51,102) 100% )",
    BACKGROUND_COLOR: "#E9ECF2",
    BACKGROUND_COLOR_BUTTON: "#A66226",

    BOX_SHADOW: "0 0 20px rgba(13, 13, 13, 0.1)",
    BOX_SHADOW_BLUE: " -3px 4px 0 rgb(102 153 255)",
    BOX_SHADOW_BUTTON: "0 10px 20px -10px #b664b0",
  };


// #D9325E
// #E9ECF2
// #D92546
// #A66226
// #0D0D0D

// #607D8B
// #FFFFFF
// #E3E3E3
// #C9C9C9
// #55585D